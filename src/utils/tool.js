...
tool.dictTypeData = (dictValue, value) => {
  let dictTypeTree = tool.dictDataAll()
  let tree
  if (!dictTypeTree) {
    return '需重新登录'
  }
  if (dictValue.indexOf('.') !== -1) {
    const arr = dictValue.split('.')
    let i = 0
    while (i !== arr.length - 1) {
      tree = dictTypeTree.find(item => item.dictValue === arr[i])
      dictTypeTree = tree.children
      i++
    }
    tree = tree.children.find(item => item.dictValue === arr[i])
  } else {
    tree = dictTypeTree.find((item) => item.dictValue === dictValue)
  }
  if (!tree) {
    return '无此字典'
  }
